 ...
 <?php if($sim=="ok") { ?>
  ...
  <input name="thn" type="text" value="2015">
                                    ...
                                    <tbody>
                                       <?php 
									   $no=0;
									   //"select * from komoditi where IdKecamatan='".$_POST['IdKecamatan']."' and jenis_komoditi <> '3' order by jenis_komoditi, NamaKomoditi asc"
									   
									   $query=mysql_query("SELECT * FROM komoditi WHERE EXISTS (SELECT idKecamatan FROM komoditi WHERE tbl_user.id_Kecamatan=komoditi.idKecamatan AND tbl_user.id_User='".$_POST['IdKecamatan']."')");
